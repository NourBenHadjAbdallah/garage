      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-14 sm:px-6 lg:px-8">
        {order === undefined ? (
          ...
          <div className="text-center">
            <h1 className="font-heading text-2xl font-bold uppercase tracking-tight">Commande introuvable</h1>
          ...
          <>
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                ...
                a bien été enregistrée. Notre équipe vérifiera votre paiement puis expédiera votre commande.
              </p>
            </div>
            ...
            <div className="mt-8 rounded-2xl border border-border p-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-heading text-sm font-semibold uppercase tracking-wide">Récapitulatif</h2>
              </div>
              <ul className="mt-4 space-y-3">
                {order.items.map((item) => (
                  <li key={item.id} className="flex items-center gap-3">
                    <img src={item.image || '/placeholder.svg'} alt={item.name} className="h-14 w-12 rounded-md object-cover" />
                    <div className="flex flex-1 items-center justify-between">
                      <span className="text-sm">{item.name} × {item.quantity}</span>
                      <span className="text-sm">{formatTND(item.price * item.quantity)}</span>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-right">{order.address}, {order.city}, {order.governorate}</span>
                </div>
                <Separator />
            ...
            <div className="mt-6 flex items-center gap-3 rounded-xl bg-muted/50 p-4 text-sm text-muted-foreground">
              ...
              <p>Vous recevrez un appel ou un message de confirmation avant l&apos;expédition. Délai estimé : 2 à 4 jours ouvrables.</p>
            </div>

            <Button asChild className="mt-6 w-full font-heading uppercase tracking-widest">
